...
local function UpdateESP(player)
    ...
    if Config.LimitDistance and dist > Config.LimitDistance then
        -- За пределами лимита — отключаем ESP
        for k, v in pairs(esp) do
            if typeof(v) == "Instance" and v:IsA("Highlight") then
                v.Enabled = false
            elseif typeof(v) == "userdata" then
    ...
    if Config.FilledBox then
        esp.Box.Filled = true
        esp.Box.Color = Config.FilledBoxColor
		esp.Box.Transparency = Config.FilledBoxTransparency
    end
    ...
    esp.Name.Outline = false --Config.NamesOutline
    ...
    esp.Studs.Outline = false --Config.StudsOutline
    ...
    esp.Tool.Outline = false --Config.ToolOutline

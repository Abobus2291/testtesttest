...
-- // Main
local PrevConfig = {
    EnabledESP = Config.EnabledESP,
    Box = Config.Box,
    BoxOutline = Config.BoxOutline,
    Names = Config.Names,
    HealthBar = Config.HealthBar,
    StudsText = Config.StudsText,
    ToolText = Config.ToolText,
    Tracers = Config.Tracers,
    Chams = Config.Chams,
}

-- // Функция обновления видимости/включения ESP-объектов на основе конфигурации
local function UpdateVisibility(player)
    local esp = EspCache[player]
    if not esp then return end

    local enabledESP = Config.EnabledESP
    esp.Box.Visible = enabledESP and Config.Box
    esp.BoxOutline.Visible = enabledESP and Config.BoxOutline
    esp.Name.Visible = enabledESP and Config.Names
    esp.Health.Visible = enabledESP and Config.HealthBar
    esp.HealthOutline.Visible = enabledESP and Config.HealthBar
    esp.HealthText.Visible = enabledESP and Config.HealthBar
    esp.Studs.Visible = enabledESP and Config.StudsText
    esp.Tool.Visible = enabledESP and Config.ToolText
    esp.Tracer.Visible = enabledESP and Config.Tracers
    esp.Chams.Enabled = enabledESP and Config.Chams
end

RunService.RenderStepped:Connect(function()
    -- Проверяем изменения конфигурации (EnabledESP и др.)
    if PrevConfig.EnabledESP ~= Config.EnabledESP or
       PrevConfig.Box ~= Config.Box or
       PrevConfig.BoxOutline ~= Config.BoxOutline or
       PrevConfig.Names ~= Config.Names or
       PrevConfig.HealthBar ~= Config.HealthBar or
       PrevConfig.StudsText ~= Config.StudsText or
       PrevConfig.ToolText ~= Config.ToolText or
       PrevConfig.Tracers ~= Config.Tracers or
       PrevConfig.Chams ~= Config.Chams then

        -- Обновляем видимость у всех ESP
        for player, _ in pairs(EspCache) do
            UpdateVisibility(player)
        end

        -- Обновляем кэш предыдущих значений
        PrevConfig.EnabledESP = Config.EnabledESP
        PrevConfig.Box = Config.Box
        PrevConfig.BoxOutline = Config.BoxOutline
        PrevConfig.Names = Config.Names
        PrevConfig.HealthBar = Config.HealthBar
        PrevConfig.StudsText = Config.StudsText
        PrevConfig.ToolText = Config.ToolText
        PrevConfig.Tracers = Config.Tracers
        PrevConfig.Chams = Config.Chams
    end
